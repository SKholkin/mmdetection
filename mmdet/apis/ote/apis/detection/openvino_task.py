...
class OpenVINODetectionTask(IInferenceTask, IEvaluationTask):
    ...
    def optimize(self,
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            xml_path = os.path.join(tempdir, "openvino.xml")
            bin_path = os.path.join(tempdir, "openvino.bin")
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            save_model(compressed_model, tempdir)
            output_model.set_data("openvino.xml", open(os.path.join(tempdir, "model.xml"), "rb").read())
            output_model.set_data("openvino.bin", open(os.path.join(tempdir, "model.bin"), "rb").read())
